...
# takes paths of directory with data files and output directory as command line inputs,
# and saves a .csv file containing information from these files as well as
# information obtained through web scraping in provided output directory
main <- function(){
  ...
  countries_more_data <- get_table_as_df(as.character(URLs['pop']), 
                                         as.character(xpaths['pop']))
  countries_more_data <- subset(countries_more_data,
                                select=c('Country..or.dependency.', 
                                         'Population..2020.', 
                                         'Land.Area..Km².', 
                                         'Density..P.Km².', 
                                         'Urban.Pop..'))
  colnames(countries_more_data) <- c('Country', 
                                     'Population', 
                                     'Land_Area_km2', 
                                     'Density_per_km2', 
                                     'Urban_Population')
  ...
  i_hdi <- get_table_as_df(as.character(URLs['hdi']), 
                           as.character(xpaths['hdi']))
  ...
  gdp_pc <- get_table_as_df(as.character(URLs['gdp']), 
                            as.character(xpaths['gdp']))
  gdp_pc <- gdp_pc[,c('Country.Territory', 'US.')]
  colnames(gdp_pc) <- c('Country', 'GDP_Per_Capita_USD')

  # government budget per capita
  gb_pc <- get_table_as_df(as.character(URLs['gbp']), 
                           as.character(xpaths['gbp']))
  ...
  below_poverty <- get_table_as_df(as.character(URLs['pov']), 
                                   as.character(xpaths['pov']))
  below_poverty <- below_poverty[,-which(colnames(below_poverty) == 'Year')]
  colnames(below_poverty) <- c('Country', 
  ...
  # gini index
  inequality <- get_table_as_df(as.character(URLs['equ']), 
                                as.character(xpaths['equ']))
  ...
  # only include countries present in all individual dataframes
  df_int <- merge_by_country(c(excel_dfs, csv_dfs), data_year, select_year=TRUE)
  # remove year column as it would be added again on next merge_by_country call
  df_int <- df_int[,-2]
  df_processed <- merge_by_country(c('df_int', 
                                     json_dfs, 
                                     'countries_more_data', 
                                     'i_hdi', 
                                     'gdp_pc',
                                     'below_poverty',
                                     'inequality'),
                                   data_year, select_year=FALSE, env=main_env)
  # budget values typically from 2017, which may not be relevant for all years
  ...
  write.csv(df_processed, 
            paste(out_dir, '/data', data_year, '_processed.csv', sep=''), 
            row.names=FALSE)
  cat(paste0('Processing Finished!\n', out_file, 
             ' is now available in ', out_dir))
}
